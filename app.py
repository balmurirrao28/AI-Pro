...
app=FastAPI(title="ScheduleAI",version="4.0")
...
GOOGLE_ID=os.getenv("GOOGLE_CLIENT_ID","");GOOGLE_SECRET=os.getenv("GOOGLE_CLIENT_SECRET","");BASE=os.getenv("APP_URL","").rstrip("/")
oauth=OAuth()
if GOOGLE_ID and GOOGLE_SECRET:
 oauth.register(name="google",client_id=GOOGLE_ID,client_secret=GOOGLE_SECRET,server_metadata_url="https://accounts.google.com/.well-known/openid-configuration",client_kwargs={"scope":"openid email profile https://www.googleapis.com/auth/calendar"})
MONTHS={m.lower():i for i,m in enumerate("January February March April May June July August September October November December".split(),1)};DAYS={d.lower():i for i,d in enumerate("Monday Tuesday Wednesday Thursday Friday Saturday Sunday".split())}
...
def parse_date(q):
 ...
 if m:
  try:
   d=datetime(b.year,MONTHS[m[1]],int(m[2]),tzinfo=TZ);d=d.replace(year=d.year+1) if d.date()<b.date() else d;return ds(d)
  except:return None
...
def find_event(q,old=None):
 ...
 scored.sort(key=lambda x:(-x[0],x[1]["date"],x[1]["time"]));return scored[0][1] if scored and scored[0][0] else (data[0] if len(data)==1 else None)
...
async def google_login(request:Request):
 if not GOOGLE_ID or not GOOGLE_SECRET:return HTMLResponse("<h3>Google Calendar is not configured.</h3><p>Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in Render Environment Variables.</p>",503)
...
async def google_callback(request:Request):
 token=await oauth.google.authorize_access_token(request);request.session["google_token"]=token["access_token"];request.session["google_user"]=token.get("userinfo",{});return RedirectResponse("/")
@app.get("/auth/logout")
...
PAGE=r'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ScheduleAI</title><style>:root{--bg:#f5f7fc;--ink:#172033;--muted:#778199;--line:#e4e8f0;--a:#635bdf;--dark:#11172a;--soft:#f0f1f8}*{box-sizing:border-box}body{margin:0;font-family:Inter,system-ui,Arial;background:var(--bg);color:var(--ink)}.app{display:flex;min-height:100vh}.side{width:252px;background:linear-gradient(180deg,#11172a,#171d35);color:#fff;padding:28px 18px}.brand{font-size:25px;font-weight:850;margin:2px 8px 34px}.brand span{color:#9a91ff}.nav{padding:12px 14px;color:#aeb7cf;border-radius:12px;margin:6px 0;font-size:14px}.active{background:#2b264d;color:#fff}.tip{margin-top:28px;padding:16px;border:1px solid #303750;border-radius:16px;color:#c4cada;font-size:12px;line-height:1.8}.main{width:min(1120px,100%);margin:auto;padding:30px}.top{display:flex;justify-content:space-between;align-items:center;gap:12px}.eyebrow{font-size:12px;font-weight:800;letter-spacing:.12em;color:var(--a);text-transform:uppercase}.head{font-size:38px;margin:7px 0}.sub{color:var(--muted)}.auth{display:flex;gap:8px;align-items:center}.google,.voice{border:1px solid var(--line);background:#fff;border-radius:12px;padding:11px 14px;text-decoration:none;color:var(--ink);font-weight:750;cursor:pointer}.connected{color:#16834e;font-size:12px;font-weight:800;margin:12px 0}.quick{display:grid;grid-template-columns:repeat(4,1fr);gap:11px;margin:24px 0 16px}.quick button{border:1px solid var(--line);background:#fff;border-radius:15px;padding:14px;text-align:left;cursor:pointer}.qtitle{font-weight:800}.qdesc{display:block;color:var(--muted);font-size:11px;margin-top:4px}.chat{background:#fff;border:1px solid var(--line);border-radius:24px;min-height:450px;padding:22px;box-shadow:0 18px 50px #17203a0d}.welcome{display:flex;gap:13px;background:linear-gradient(135deg,#f3f2ff,#f7f8fc);border:1px solid #e6e4fb;padding:18px;border-radius:18px}.avatar{width:38px;height:38px;border-radius:12px;background:linear-gradient(135deg,var(--a),#8a7ff0);color:#fff;display:grid;place-items:center;font-weight:900}.welcome strong{display:block;margin-bottom:4px}.welcome span{color:var(--muted);font-size:13px;line-height:1.5}.msg{max-width:84%;padding:14px 17px;border-radius:17px;margin:10px 0;line-height:1.55;white-space:pre-wrap;font-size:14px}.bot{background:var(--soft)}.user{margin-left:auto;background:linear-gradient(135deg,var(--a),#766de7);color:#fff}.events{display:grid;gap:9px}.event{display:flex;align-items:center;gap:12px;padding:12px;background:#fff;border:1px solid #e6e9f0;border-radius:14px}.datebox{width:54px;text-align:center;background:#f1f0ff;border-radius:10px;padding:7px 4px;color:var(--a);font-weight:850;font-size:11px}.eventmain{flex:1}.eventtitle{font-weight:800;font-size:13px}.meta{color:var(--muted);font-size:11px;margin-top:3px}.tag{font-size:10px;padding:5px 8px;border-radius:20px;background:#f0f1f5;color:#626d83}.composer{display:flex;gap:8px;margin-top:14px;background:#fff;border:1px solid var(--line);padding:7px;border-radius:17px}.composer input{flex:1;border:0;background:transparent;padding:13px;font-size:14px;outline:none}.send{border:0;background:var(--a);color:#fff;border-radius:12px;padding:0 20px;font-weight:850}.foot{text-align:center;color:#929bae;font-size:11px;margin-top:11px}@media(max-width:820px){.side{display:none}.main{padding:22px 14px}.head{font-size:30px}.quick{grid-template-columns:repeat(2,1fr)}.top{align-items:flex-start}.auth{flex-direction:column;align-items:flex-end}}@media(max-width:480px){.quick{grid-template-columns:1fr}}</style></head><body><div class="app"><aside class="side"><div class="brand">Schedule<span>AI</span></div><div class="nav active">✦ &nbsp;Assistant</div><div class="nav">▣ &nbsp;30-Day Schedule</div><div class="nav">◷ &nbsp;Agent Tools</div><div class="tip"><b>Try asking</b><br>What do I have tomorrow?<br>Am I free Friday afternoon?<br>Add a meeting tomorrow at 3 PM.<br>Move my meeting from 2 PM to 4 PM.<br>What is today's date?</div></aside><main class="main"><div class="top"><div><div class="eyebrow">Agentic Schedule Assistant</div><div class="head">Your schedule, understood.</div><div class="sub">RAG + Google Calendar + voice commands.</div></div><div class="auth"><button class="voice" onclick="voice()">🎙 Voice</button><a class="google" href="/auth/google">G&nbsp; Connect Google Calendar</a></div></div><div id="gstatus"></div><div class="quick"><button onclick="ask('What do I have today?')"><span class="qtitle">📅 Today</span><span class="qdesc">See today’s events</span></button><button onclick="ask('What do I have tomorrow?')"><span class="qtitle">Tomorrow</span><span class="qdesc">Plan the next day</span></button><button onclick="ask('Am I free Friday afternoon?')"><span class="qtitle">Free time</span><span class="qdesc">Check Friday afternoon</span></button><button onclick="ask('Show my meetings')"><span class="qtitle">Meetings</span><span class="qdesc">Find scheduled meetings</span></button></div><div id="chat" class="chat"><div class="welcome"><div class="avatar">AI</div><div><strong>Hi! I’m ScheduleAI.</strong><span>Connect Google Calendar to include your real calendar, or use the sample 30-day schedule. You can also use your microphone for hands-free commands.</span></div></div></div><div class="composer"><input id="q" autocomplete="off" placeholder="Ask: What do I have tomorrow?"><button class="voice" onclick="voice()">🎙</button><button class="send" onclick="send()">Send</button></div><div class="foot">FastAPI • ChromaDB • Agentic RAG • Google Calendar • Voice</div></main></div><script>function ask(t){q.value=t;send()}function add(t,c){const x=document.createElement('div');x.className='msg '+c;x.textContent=t;chat.appendChild(x);x.scrollIntoView({behavior:'smooth'});return x}function card(items){const x=document.createElement('div');x.className='msg bot';const w=document.createElement('div');w.className='events';items.forEach(e=>{const r=document.createElement('div');r.className='event';const d=new Date(e.date+'T00:00:00');r.innerHTML='<div class="datebox">'+d.toLocaleDateString('en-US',{month:'short'})+'<br>'+d.getDate()+'</div><div class="eventmain"><div class="eventtitle">'+esc(e.title)+'</div><div class="meta">'+e.date+' • '+t12(e.time)+' • '+e.duration+' min</div></div><div class="tag">'+esc(e.type)+'</div>';w.appendChild(r)});x.appendChild(w);chat.appendChild(x);x.scrollIntoView({behavior:'smooth'})}function t12(t){let [h,m]=t.split(':');h=+h;return(h%12||12)+':'+m+' '+(h<12?'AM':'PM')}function esc(s){return String(s).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}async function send(){const v=q.value.trim();if(!v)return;q.value='';add(v,'user');const b=add('Thinking…','bot');try{const r=await fetch('/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:v})});const d=await r.json();b.remove();if(Array.isArray(d.data)&&d.data.length)card(d.data);else add(d.answer||'No answer returned.','bot')}catch(e){b.remove();add('Unable to reach the assistant. Please try again.','bot')}}q.addEventListener('keydown',e=>{if(e.key==='Enter')send()});function voice(){const S=window.SpeechRecognition||window.webkitSpeechRecognition;if(!S){add('Voice input is not supported in this browser. Try Chrome or Edge.','bot');return}const r=new S();r.lang='en-IN';r.interimResults=false;r.onstart=()=>add('🎙 Listening…','bot');r.onresult=e=>{q.value=e.results[0][0].transcript;send()};r.onerror=()=>add('I could not hear that. Please try again.','bot');r.start()}async function googleStatus(){try{const r=await fetch('/api/google-calendar');const d=await r.json();if(d.connected){gstatus.innerHTML='<div class="connected">✓ Google Calendar connected — '+d.events.length+' upcoming events loaded.</div>';if(d.events.length){const mapped=d.events.map(e=>{const v=(e.start||{}).dateTime||e.start?.date||'';const p=v.replace('Z','').split('T');return{title:e.summary||'Google Calendar event',date:p[0]||'',time:(p[1]||'00:00').slice(0,5),duration:60,type:'Google Calendar'}});card(mapped.slice(0,12))}}}catch(e){}}googleStatus();</script></body></html>'''
...
@app.get("/health")
def health():return {"status":"ok","events":col.count(),"tools":["get_schedule","update_schedule"],"google_calendar":bool(GOOGLE_ID and GOOGLE_SECRET),"voice":"browser SpeechRecognition"}
